...
<div class="p-4">
    <p class="text-lg mt-4">Apakah anda ingin menghapus role <b>{{$role->name}}</b>?</p>
    <form class="w-full max-w-full" action="{{route('dashboard.admin.roles.destroy', ['role' => $role->id])}}" method="POST">
        @csrf
        @method('DELETE')
        @if($errors->any())
