...
<div class="p-4">
    <p class="text-lg mt-4">Apakah anda ingin menghapus client <b>{{$client->name}}</b>?</p>
    <form class="w-full max-w-full" action="{{route('dashboard.admin.oauth.destroy', ['oauth' => $client->id])}}" method="POST">
        @csrf
        @method('DELETE')
        @if($errors->any())
