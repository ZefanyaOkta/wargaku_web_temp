...
<x-modal-component title={{$title}} modalId={{$modalId}}>
    <div class="p-4">
        <p class="text-lg mt-4">Apakah anda ingin menghapus Kategori <b>{{$kategori->name}}</b>?</p>
        <form class="w-full max-w-full" action="{{route('dashboard.admin.categories.destroy', ['category' => $kategori->id])}}" method="POST">
            @csrf
            @method('DELETE')
            @if($errors->any())
